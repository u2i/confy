...
RSpec.describe GoogleCalendar::EventEditor do
  ...
  let(:event) do
    Google::Apis::CalendarV3::Event.new(start: WithDatetime.new(start_time),
                                        end: WithDatetime.new(end_time), id: event_id)
  end
  let(:service) { double('service', get_event: event, update_event: nil) }

  subject { described_class.new(service, :user_email) }

  before do
    allow_any_instance_of(GoogleCalendar::Client).to receive(:calendar_service) { service }
